<x-admin-layout>
    ...
    <div class="w-full text-sm">
        <div class="mb-5">
            <h1 class="text-3xl font-bold">Edit Tipe Kendaraan</h1>
            <h2 class="text-gray-500">Silakan mengubah nomor dan deskripsi tipe kendaraan dibawah ini</h2>
        </div>
        <div class="bg-white w-full rounded-lg p-5">
            <form action="/lainnya/vehicle-type/edit" method="POST">
                @csrf
                <input type="hidden" name="id" value="{{ $vehicletype->id }}">
                <div class="mb-5">
                    <label for="su_number" class="text-sm font-semibold">su Number</label>
                    <input type="text" name="name" id="su_number" value="{{ $vehicletype->name }}"
                ...
                <div class="mb-5">
                    ...
                    <textarea name="desc" id="description" rows="4"
                        class="w-full border border-gray-300 rounded-lg p-2 mt-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        placeholder="Masukkan Deskripsi su" required>{{ $vehicletype->desc }}</textarea>
                </div>
                <button type="submit"
