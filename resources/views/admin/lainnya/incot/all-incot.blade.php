<x-admin-layout>
    ...
    <div class="w-full ">
        <x-navbar>
            <x-slot:createLink>/lainnya/incot/tambah</x-slot:createLink>
        ...
        <div class="rounded-xl shadow overflow-hidden border bg-white border-gray-200 overflow-x-scroll">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50  text-gray-600 text-sm font-semibold">
                    <tr>
                        <th class="px-6 py-3 text-left">Inisial incot</th>
                ...
                <tbody class="divide-y divide-gray-100 text-sm">
                    @if (count($incots) > 0)
                        @foreach ($incots as $incot)
                            <tr class="bg-white">
                                <td class="px-6 py-4 font-bold text-gray-700">{{ $incot->name }}</td>
                                <td class="px-6 py-4">{{ $incot->desc }}</td>
                                <td class="px-6 py-4 ">
                                    <div class="flex gap-2 items-center justify-start">
                                        <a href="/lainnya/incot/{{ $incot->id }}/edit"
                                            class="bg-blue-500 hover:bg-blue-700 cursor-pointer text-white font-bold py-1 px-4 inline-block rounded">Edit</a>
                                        <a href="/lainnya/incot/{{ $incot->id }}/hapus"
